...
export class FileQueueItem {
  ...
  private readonly progress: UploadProgress;
  ...
  constructor(dragUploader: DragUploader, file: File, override: Action) {
    ...
    this.progress.indicator.setAttribute('colspan', String(this.dragUploader.fileListColumnCount - this.row.querySelectorAll('td').length));
    this.row.append(this.progress.indicator);
  ...
  public uploadError(response: XMLHttpRequest): void {
    ...
    this.progress.finalize(SeverityEnum.error, TYPO3.lang['file_upload.uploadFailed'].replace(/\{0\}/g, this.file.name));
  ...
  public updateProgress(event: ProgressEvent): void {
    ...
    this.progress.update(percentage, `${TYPO3.lang['file_upload.upload-in-progress']} ${percentage}%`);
  ...
  public uploadSuccess(data: { upload?: UploadedFile[], messages?: FlashMessage[] }): void {
    if (data.upload) {
      ...
      this.progress.finalize(SeverityEnum.ok, TYPO3.lang['file_upload.uploadSucceeded']);
