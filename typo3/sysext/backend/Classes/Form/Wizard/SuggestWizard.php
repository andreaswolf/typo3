...
class SuggestWizard {
	...
	protected function currentBackendUserMayAccessTable($tableConfig) {
		...
		if (
		    (bool)$tableConfig['ctrl']['rootLevel'] === FALSE
	 ...
	 * @return array
	 */
	protected function getConfigurationForTable($queryTable, $wizardConfig, $TSconfig, $table, $field) {
	 ...
	 * @return string
	 */
	protected function getWhereClause($fieldConfig) {
		if (!isset($fieldConfig['foreign_table'])) {
			return '';
		}

		$foreignTableWhere = $fieldConfig['foreign_table_where'];
		// strip ORDER BY clause
		$foreignTableWhere = trim(preg_replace('/ORDER[[:space:]]+BY.*/i', '', $foreignTableWhere));
